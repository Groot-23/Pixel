...
public class GameListener implements Listener{
	...
	public Game getGame(World world) {
		Arena arena = miniGame.worldProvider.getArena(world.getUID());
		if(arena != null) {
	...
	public void onEntityDamageByEntity(EntityDamageByEntityEvent event) {
		if(event.getEntity() instanceof Player) {
			if(event.getDamager() instanceof Player) {
				MinG.setLastAttacker((Player)event.getEntity(), (Player)event.getDamager());
			} else if(event.getDamager() instanceof Projectile) {
				ProjectileSource source = ((Projectile)event.getDamager()).getShooter();
				if(source != null && source instanceof Player) {
					MinG.setLastAttacker((Player)event.getEntity(), (Player)source);
				}
			}
		}
	}
